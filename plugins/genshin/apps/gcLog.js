...
export class gcLog extends plugin {
  ...
  async help () {
    await this.e.reply(this.e.segment.image(`file:///${_path}/resources/logHelp/记录帮助.png`))
  }
  ...
  async helpPort () {
    ...
    if (['电脑', 'pc'].includes(msg)) {
      await this.e.reply(this.e.segment.image(`file:///${_path}/resources/logHelp/记录帮助-电脑.png`))
    } else if (['安卓'].includes(msg)) {
      await this.e.reply(`安卓抽卡记录获取教程：${this.androidUrl}`)
    } else if (['苹果', 'ios'].includes(msg)) {
      await this.e.reply(this.e.segment.image(`file:///${_path}/resources/logHelp/记录帮助-苹果.png`))
    }
  }
